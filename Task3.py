...
def player_entry():
    global play_active
    while True:
        move = input("Please enter the number of the field: ")
        ...
        try:
            ...
            if move >= 1 and move <= 9:
                if playboard[move] == 'X' or playboard[move] == 'O':
                    print("This field is busy, choose another number!")
                else:
                    return move
            else:
...
while play_active:
    print()
    print ("Player " + curent_player + " it is your move ")
    mover = player_entry()
    if mover:
